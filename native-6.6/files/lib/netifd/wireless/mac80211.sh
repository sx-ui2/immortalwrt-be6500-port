...
drv_mac80211_setup() {
	# All three BE6500 radios share one QSDK wiphy and one pair of
	# hostapd/wpa_supplicant ubus services.  netifd normally starts radio
	# scripts in parallel, which makes their config_set/remove/create sequences
	# race and intermittently returns -EBUSY/-ENFILE.  Serialize the complete
	# setup transaction on this board only.
	if [ "$(cat /tmp/sysinfo/board_name 2>/dev/null)" = "jdcloud,be6500" ]; then
		# Three logical radios share one hostapd/ucode process.  ACS/DFS can
		...
		while ! mkdir "$be6500_radio_lock" 2>/dev/null; do
			# Recover a lock left behind by a killed netifd radio worker.  Never
			# remove a lock while its recorded owner is still alive.
			local be6500_lock_owner
			be6500_lock_owner="$(cat "$be6500_radio_lock/pid" 2>/dev/null)"
			case "$be6500_lock_owner" in
				''|*[!0-9]*) rmdir "$be6500_radio_lock" 2>/dev/null || true ;;
				*) kill -0 "$be6500_lock_owner" 2>/dev/null || {
					rm -f "$be6500_radio_lock/pid"
					rmdir "$be6500_radio_lock" 2>/dev/null || true
				} ;;
			esac
			sleep 1
			be6500_radio_wait=$((be6500_radio_wait + 1))
			[ "$be6500_radio_wait" -lt 120 ] || {
				echo "Timed out waiting for the BE6500 radio setup lock"
				wireless_setup_failed RADIO_SETUP_LOCK_TIMEOUT
				return 1
			}
		done
		printf '%s\n' "$$" >"$be6500_radio_lock/pid"
		trap 'rm -f /tmp/be6500-radio-setup.lock/pid; rmdir /tmp/be6500-radio-setup.lock 2>/dev/null' EXIT INT TERM
		# A transient QSDK interface allocation failure must not leave a radio
		# permanently inactive.  Let netifd retry the affected logical radio.
		wireless_set_retry 3
	fi
	...
	# The QSDK driver advertises ACS for the shared wiphy but cannot run QACS
	# on the BE6500 2.4 GHz pdev.  Keep UCI/UI set to auto and use the channel
	# selected by our post-start survey helper.  On the first pass only, bring
	# the pdev up briefly on channel 6 so it can scan, then the helper moves it
	# to the least congested non-overlapping channel without changing UCI.
	if [ "$(cat /tmp/sysinfo/board_name 2>/dev/null)" = "jdcloud,be6500" ] &&
		[ "$radio" = "0" ]; then
		case "$channel" in
			auto|0|''|acs_survey)
				local be6500_auto_channel_file=/var/run/be6500-radio0-auto-channel
				local be6500_auto_channel
				be6500_auto_channel="$(cat "$be6500_auto_channel_file" 2>/dev/null)"
				case "$be6500_auto_channel" in
					1|6|11) channel="$be6500_auto_channel" ;;
					*) channel=6; be6500_auto_channel_pending=1 ;;
				esac
				auto_channel=0
				;;
		esac
	...
	# A 160 MHz chandef beginning at channel 149 would extend through channel
	# 177, which is outside the regulatory/channel data used by this board.
	# Clamp invalid fixed primaries immediately before hostapd is configured.
	if [ "$(cat /tmp/sysinfo/board_name 2>/dev/null)" = "jdcloud,be6500" ] &&
		echo "$htmode" | grep -Eq '(HT|VHT|HE|EHT)160$'; then
		be6500_mode="$(uci -q get wireless.main.freq_mode 2>/dev/null)"
		be6500_allowed=""
		be6500_fallback=36
		case "$radio:$be6500_mode" in
			1:1) be6500_allowed="36 40 44 48 52 56 60 64"; be6500_fallback=36 ;;
			2:1) be6500_allowed="100 104 108 112 116 120 124 128"; be6500_fallback=100 ;;
			1:0) be6500_allowed="36 40 44 48 52 56 60 64 100 104 108 112 116 120 124 128"; be6500_fallback=36 ;;
		esac
		case " $be6500_allowed " in
			*" $channel "*) ;;
			*) case "$channel" in
					auto|0|'') ;;
					149|153|157|161|165)
						# Keep the user's high-band primary channel and select the
						# widest legal mode for it instead of rejecting the radio.
						htmode="${htmode%160}80" ;;
					*) channel="$be6500_fallback" ;;
					esac ;;
		esac
	fi
	...
	# QSDK rejects an empty supplicant config array with EINVAL.  AP-only
	# radios do not need a supplicant transaction at all.
	[ -x /usr/sbin/wpa_supplicant ] && [ -n "$wpa_supp_init" ] && \
		wpa_supplicant_set_config "$phy" "$radio"

	# QSDK multi-radio wiphys require the AP netdev to carry a radio mask
	# before hostapd takes ownership of it.  Upstream hostapd normally creates
	# AP netdevs itself, but that loses the pdev selection on dual-MAC QCN92xx
	# hardware and makes config_set time out with HOSTAPD_START_FAILED.
	# Pre-create only the primary AP of a numbered radio; hostapd will then
	# recreate it while retaining the requested radio association.
	if [ -x /usr/sbin/hostapd ] && [ -n "$phy_suffix" ] && \
	   [ -n "$ap_ifname" ] && [ ! -e "/sys/class/net/$ap_ifname" ]; then
		wdev_tool "$phy$phy_suffix" set_config \
			"{\"$ap_ifname\":{\"mode\":\"ap\"}}" "$ap_ifname" || {
			wireless_setup_failed AP_NETDEV_CREATE_FAILED
			return 1
		}
	fi
	...
	# Do not let the next numbered radio start its scan while this AP is still
	# in ACS/HT_SCAN.  QSDK exposes all pdevs through one wiphy, so overlapping
	# scans return -EBUSY and can leave an SSID visible but non-associable.
	if [ -n "$be6500_radio_lock" ] && [ -n "$ap_ifname" ]; then
		local be6500_ap_wait=0
		local be6500_ap_status
		# The OEM QCN9224 firmware can run a 60 second CAC even on the
		# lower 5 GHz path.  Do not mark netifd failed while hostapd is
		# still completing that valid start-up sequence.
		while [ "$be6500_ap_wait" -lt 90 ]; do
			be6500_ap_status="$(ubus -S call "hostapd.$ap_ifname" get_status 2>/dev/null | \
				jsonfilter -e '@.status' 2>/dev/null)"
			[ "$be6500_ap_status" = "ENABLED" ] && break
			sleep 1
			be6500_ap_wait=$((be6500_ap_wait + 1))
		done
		[ "$be6500_ap_status" = "ENABLED" ] || {
			echo "BE6500 AP $ap_ifname did not become ready"
			# Remove only the incomplete VAP.  Other logical radios on the shared
			# wiphy stay up and the current one is recreated by netifd's retry.
			ubus -S call hostapd config_remove \
				"{\"iface\":\"$ap_ifname\"}" >/dev/null 2>&1 || true
			ip link set "$ap_ifname" down >/dev/null 2>&1 || true
			iw dev "$ap_ifname" del >/dev/null 2>&1 || true
			rm -f "$be6500_radio_lock/pid"
			rmdir "$be6500_radio_lock" 2>/dev/null
			wireless_setup_failed AP_ENABLE_TIMEOUT
			return 1
		}
	fi
	if [ -n "$be6500_radio_lock" ]; then
		rm -f "$be6500_radio_lock/pid"
		rmdir "$be6500_radio_lock" 2>/dev/null
	fi
	wireless_set_up
	if [ "$be6500_auto_channel_pending" = "1" ] &&
		[ -x /usr/libexec/be6500-radio0-auto-channel ]; then
		(sleep 2; /usr/libexec/be6500-radio0-auto-channel) >/dev/null 2>&1 &
	fi
}
