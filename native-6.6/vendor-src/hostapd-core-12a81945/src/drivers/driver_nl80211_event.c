...
#define C2S(x) case x: return #x;
	...
	C2S(NL80211_CMD_STOP_BGRADAR_DETECT)
	C2S(NL80211_CMD_INTERFERENCE_DETECT)
	...
	C2S(NL80211_CMD_TRIGGER_SMD_DISCOVERY)
				 ...
				 int finished)
{
	...
	if (mcst)
		data.ch_switch.mcst = nla_get_u32(mcst);
	else
		data.ch_switch.mcst = 0;
				    ...
				    struct nlattr *rx_link_removal_params)
{
	...
	if (!link_removal[NL80211_CU_ATTR_MLD_LIST]) {
