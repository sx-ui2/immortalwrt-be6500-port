...
static void wpa_driver_nl80211_deinit(struct i802_bss *bss)
{
	...
	eloop_cancel_timeout(wpa_driver_nl80211_scan_timeout, drv, ELOOP_ALL_CTX);
	...
	if (freq->vht_enabled ||
		...
		if (freq->eht_enabled && freq->punct_bitmap) {
			if (!bss->drv->puncturing) {
				wpa_printf(MSG_INFO,
					   "nl80211: Puncture bitmap is set but the driver does not support puncturing");
				return -EINVAL;
			}
...
static int nl80211_reset_afc(void *priv, u8 link_id)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Resetting AFC");
	...
	if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_VENDOR)) ||
	    nla_put_u32(msg, NL80211_ATTR_VENDOR_ID, OUI_QCA) ||
	    nla_put_u32(msg, NL80211_ATTR_VENDOR_SUBCMD,
			QCA_NL80211_VENDOR_SUBCMD_AFC_RESET)) {
		goto error;
	}
	...
	if (ret) {
		wpa_printf(MSG_DEBUG,
			   "nl80211: AFC reset failed err=%d (%s)",
			   ret, strerror(-ret));
	}
	return ret;
error:
	nlmsg_free(msg);
	wpa_printf(MSG_DEBUG, "nl80211: Could not reset AFC on link %d", link_id);
	return ret;
}
