...
static int wpa_cli_cmd_smd_list_prepared(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
...
static int wpa_cli_cmd_set_bgscan_freq(struct wpa_ctrl *ctrl, int argc,
				       char *argv[])
{
	char cmd[2048];
	int res;
	...
	if (argc < 2) {
		printf("Invalid SET_BGSCAN_FREQ command: needs at least two arguments\n"
		       "(network id and frequency list or 'clear')\n");
		return -1;
	}
	...
	res = os_snprintf(cmd, sizeof(cmd),
			  "SET_NETWORK %s bgscan_freq ", argv[0]);
	if (os_snprintf_error(sizeof(cmd), res) ||
	    res < 0 || (size_t)res >= sizeof(cmd)) {
		printf("Too long SET_NETWORK command.\n");
		return -1;
	...
	for (i = 1; i < (size_t)argc; i++) {
		...
		add = os_snprintf(cmd + (size_t)res, sizeof(cmd) - (size_t)res,
				  "%s%s", prefix, argv[i]);
		if (os_snprintf_error(left, add)) {
			printf("Too long SET_NETWORK command.\n");
			return -1;
...
static char ** wpa_cli_complete_roam(const char *str, int pos)
{
	int arg = get_cmd_arg_num(str, pos);
	char **res = NULL;
	...
	switch (arg) {
	case 1:
		/* Complete target BSSID from the BSS table */
		res = cli_txt_list_array(&bsses);
		break;
	case 2:
		/* Suggest "ST" as optional second argument for SMD BSS transition */
		res = os_malloc(2 * sizeof(char *));
		if (res) {
			res[0] = os_strdup("ST");
			res[1] = NULL;
		}
...
static const struct wpa_cli_cmd wpa_cli_commands[] = {
	...
	{ "ml_probe_req", wpa_cli_cmd_ml_probe_req, NULL, cli_cmd_flag_none,
	  "= send Multi-Link Probe request <tx_link=id> <bssid=addr> <mld_id=id> [link_id=id]" },
	{ "roam", wpa_cli_cmd_roam, wpa_cli_complete_roam,
	  cli_cmd_flag_none,
	  "<addr> [ST] = roam to the specified BSS; ST triggers SMD BSS transition "
	  "(falls back to regular roam if SMD is not enabled or not MLO)" },
#ifdef CONFIG_MESH
...
static void wpa_cli_action_process(const char *msg)
{
	...
	if (str_starts(pos, WPA_EVENT_CONNECTED)) {
		...
		wpa_cli_exec(action_file, ifname, pos);
	} else if (str_starts(pos, DPP_EVENT_SAE_PWE)) {
		wpa_cli_exec(action_file, ifname, pos);
