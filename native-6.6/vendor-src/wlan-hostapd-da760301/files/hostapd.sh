...
# wifi-scripts creates a separate SAE password file for per-station keys.
# Qualcomm's hostapd.sh predates that helper while its mac80211.sh already
# calls it, which leaves every AP setup with a noisy "not found" failure.
# Keep the file format in sync with current OpenWrt wifi-scripts so SAE and
# mixed WPA2/WPA3 networks can use the same runtime data as upstream.
...
hostapd_set_bss_options() {
	...
	# QSDK removed WLAN virtual-port allocation from ath12k and expects the
	# selected PPE path to be passed through hostapd for every AP netdevice.
	# Direct Switch is the native Wi-Fi fast path; keep passive/active available
	# as explicit per-interface compatibility overrides.
	local ppe_vp_type
	case "$ppe_vp" in
		passive|1) ppe_vp_type=1 ;;
		active|2) ppe_vp_type=2 ;;
		ds|3|'') ppe_vp_type=3 ;;
		*) ppe_vp_type=3 ;;
	esac
	...
	append bss_conf "ppe_vp=$ppe_vp_type" "$N"
