			     ...
			     const char *fw_name, struct device *dev)
{
	...
	/*
	 * IPQ5332's factory TrustZone consumes the legacy QSDK metadata
	 * layout verbatim: ELF/program headers at offset 0 and the hash
	 * segment at its original (page-aligned) p_offset.  The generic
	 * loader normally compacts those two regions, which is accepted by
	 * newer firmware but leaves the BE6500's older secure loader in a
	 * state where PAS init succeeds and PAS auth/reset later returns
	 * -EINVAL.  Preserve the on-disk metadata layout for the root PD.
	 */
	if (of_device_is_compatible(dev->of_node, "qcom,ipq5332-q6-mpd") &&
	    phdrs[hash_segment].p_offset + hash_size <= fw->size &&
	    fw->size > ehdr_size + hash_size) {
		data = kmemdup(fw->data, fw->size, GFP_KERNEL);
		if (!data)
			return ERR_PTR(-ENOMEM);

		*data_len = fw->size;
		dev_info(dev, "using legacy IPQ5332 PAS metadata layout (%zu bytes)\n",
			 fw->size);
		return data;
	}
