...
wait_for_start:
	...
	/* The factory IPQ5332 TrustZone locks WCSS_CFG as soon as PAS starts
	 * the root PD.  Reading the optional version registers afterwards raises
	 * an XPU violation and a synchronous external abort.  QSDK 5.4 never
	 * performed these diagnostic reads, so retain that behaviour on IPQ5332.
	 */
	if (wcss->reg_base &&
	    !of_device_is_compatible(wcss->dev->of_node,
				     "qcom,ipq5332-q6-mpd"))
...
/* JDCOS/QWRT WCSS firmware consumes a separate heartbeat SMEM item in
 * addition to the bootargs item.  QSDK 5.4 allocated it before starting the
 * root PD, while the 6.6 MPD driver dropped that step despite retaining the
 * qcom,heartbeat_smem binding.  Restore only the allocation contract; the
 * debugfs address exported by the old driver is not required by firmware.
 */
static int create_heartbeat_smem(struct device *dev)
{
	...
	ret = of_property_read_u32(dev->of_node, "qcom,heartbeat_smem",
				   &smem_id);
	if (ret)
		return ret;
	...
	dev_info(dev, "heartbeat SMEM %u ready at 0x%lX (%zu bytes)\n",
		 smem_id, (uintptr_t)qcom_smem_virt_to_phys(ptr), size);
	return 0;
}
...
static int share_bootargs_to_q6(struct device *dev)
{
	...
	if (ret && ret != -ENOENT) {
		dev_err(dev, "failed to create heartbeat SMEM: %d\n", ret);
		return ret;
	}

	load_license_params_to_bootargs(dev, &boot_args);

	/* Keep a bounded, byte-exact trace of the host/Q6 contract.  The WCSS
	 * firmware currently reports only the generic err_smem_ver.2.1 string,
	 * so log the version, payload length and TLVs immediately before PAS
	 * starts it.  This does not change the data shared with the firmware.
	 */
	memcpy_fromio(&bootargs_len, boot_args.smem_elem_cnt_ptr,
		      sizeof(bootargs_len));
	bootargs_len = min_t(u16, bootargs_len + 2 * sizeof(u16),
			    sizeof(bootargs_dump));
	memcpy_fromio(bootargs_dump, boot_args.smem_base_ptr, bootargs_len);
	dev_info(dev, "SMEM %u bootargs: version=%u payload=%zu total=%zu\n",
		 smem_id, version, bootargs_len - 2 * sizeof(u16),
		 (size_t)bootargs_len);
	print_hex_dump(KERN_INFO, "q6_bootargs: ", DUMP_PREFIX_OFFSET,
		       16, 1, bootargs_dump, bootargs_len, false);
...
static int q6_wcss_probe(struct platform_device *pdev)
{
	...
	/*
	 * QSDK's ath12k AHB path can own the UserPD SMP2P interrupts and the
	 * spawn/stop state itself.  The remoteproc child must still be probed so
	 * the root Q6 loader can publish its firmware load address and size in
	 * SMEM bootargs, but registering the same four IRQs here would make the
	 * later ath12k probe fail with -EBUSY.
	 */
	if (desc->init_irq &&
